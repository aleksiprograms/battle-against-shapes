...
public class Flamethrower extends Weapon {
    ...
    public Flamethrower(TheGame game) {
        super(
                game,
                game.getResources().getTextureRegionByID(Constants.TEXTURE_FLAMETHROWER),
        ...
        particleEffect.load(
                Gdx.files.internal("particle_effects/weapon_flamethrower.p"),
    ...
    public void update(float deltaTime) {
        ...
        if (destroy && !destroyed) {
            ...
            particleEffect.setPosition(
                    game.getGameWorld().getPlayer().getFighter().getBox2DBody()
                            .getPosition().x + Constants.FLAME_X_OFFSET,
                    game.getGameWorld().getPlayer().getFighter().getBox2DBody()
                            .getPosition().y + Constants.FLAME_Y_OFFSET);
        }
        if (lastShoot > System.currentTimeMillis() - 100) {
            if (!shoot) {
                ...
                sound.loop(game.getSaveManager().getSaveData().getSoundVolume());
            }
        } else {
            shoot = false;
        ...
        if (timeFromLastShot >= shotDelay) {
            ...
            for (int i = 0; i < angles.length; i++) {
                velocity = new Vector2(MathUtils.cos(angles[i]), MathUtils.sin(angles[i]))
                        .scl(Constants.VELOCITY_FLAMETHROWER_PARTICLE.len());
                game.getGameWorld().addGameObjectToWorld(
                        game.getResources().getGameObjectPools()
                                .getFlamethrowerParticlePool().obtain(),
                        game.getGameWorld().getPlayer().getFighter().getBox2DBody()
                                .getPosition().x + Constants.FLAME_X_OFFSET,
                        game.getGameWorld().getPlayer().getFighter().getBox2DBody()
                                .getPosition().y + Constants.FLAME_Y_OFFSET,
