...
public class GameOverHud extends Hud {
    ...
    public GameOverHud(TheGame game) {
        ...
        super.add(new Label("GAME OVER", game.styles.skinLabelTitle1)).expandX().align(Align.top).colspan(2);
        super.row();
    ...
    public void init(long newScore, long newDistance) {
        labelGameModeTitle.setText(game.gameMode.name);
        if (game.gameStateManager.isNewBestScore())
            labelScoreText.setText("New best!");
        else
            labelScoreText.setText(" ");
        labelScore.setText(game.styles.getFormattedScore(newScore));

        if (game.gameStateManager.isNewBestDistance())
            labelDistanceText.setText("New best!");
        else
            labelDistanceText.setText(" ");
        labelDistance.setText(game.styles.getFormattedScore(newDistance) + " m");

        if (newScore >= game.gameMode.scoreToOneStar)
            imageStar1.setDrawable(imageStarUnlocked1);
        else
            imageStar1.setDrawable(imageStarLocked1);
        if (newScore >= game.gameMode.scoreToTwoStar)
            imageStar2.setDrawable(imageStarUnlocked2);
        else
            imageStar2.setDrawable(imageStarLocked2);
        if (newScore >= game.gameMode.scoreToThreeStar)
